...
// Throttle OTP requests hard — this is the endpoint an SMS-bombing script
// would hit first.
...
router.post("/send-otp", otpLimiter, async (req, res, next) => {
  try {
    ...
    // One active session per mobile — replace any previous one.
    ...
    res.json({ ok: true, expiresInSeconds: OTP_TTL_MS / 1000, ...devPayload });
  } catch (err) {
    next(err);
  }
});
...
router.post("/verify-otp", async (req, res, next) => {
  try {
    ...
    // If this mobile already belongs to a registered farmer, skip straight
    // to a full session token so a returning user doesn't have to re-enter
    // an eNAM ID every time they log in.
    const existingFarmer = await Farmer.findOne({ mobile });
    if (existingFarmer) {
      const token = signToken(
        { mobile, stage: "registered", farmerId: existingFarmer.farmerId, farmerDbId: existingFarmer._id.toString() },
        "30d"
      );
    ...
    res.json({ ok: true, stage: "otp_verified", token });
  } catch (err) {
    next(err);
  }
});
